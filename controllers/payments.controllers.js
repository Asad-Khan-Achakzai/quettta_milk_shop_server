...
paymentsController.getAll = async (req, res) => {
  ...
  if(obj.buyer || obj.amount || obj.unpaid || obj.type || obj.sale)
  {
    let searhItem;
    try {
      ...
      searhItem = await Payments.find(queryObject).populate('userId').skip(start*length).limit(length)
      .populate('buyer')
      counts = await Payments.countDocuments(queryObject);
    ...
    let items;
    try {
      items = await Payments.find({}).populate('userId').skip(start*length).limit(length)
      .populate('buyer')
      counts = await Payments.countDocuments({});
