  ...
  buyersController.getAll = async (req, res) => {
    ...
    if(obj.name || obj.address || obj.mobileN || obj.phoneN || obj.type || obj.otherN || obj.milkPrice || obj.yougurtPrice)
    {
      ...
      try {
        ...
        searhItem = await Buyers.find(queryObject).populate('userId').skip(start*length).limit(length);
        counts = await Buyers.countDocuments(queryObject);
    ...
    else
    {
      ...
      try {
        ...
        // const start = 0;
        // const length = 100;
        items = await Buyers.find({}).populate('userId').skip(start*length).limit(length);
        counts = await Buyers.countDocuments();
