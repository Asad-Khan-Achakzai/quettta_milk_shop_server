...
salesController.getAll = async (req, res) => {
  ...
  if(obj.buyer || obj.item || obj.quantity || obj.payment || obj.unitPrice)
  {
    ...
    try {
      ...
      searhItem = await Sales.find(queryObject).populate('userId').skip(start*length).limit(length)
      .populate('buyer')
      .populate('item');
    ...
    let items;
    try {
      items = await Sales.find({}).populate('userId').skip(start*length).limit(length)
      .populate('buyer')
      .populate('item');
