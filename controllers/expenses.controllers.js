...
expensesController.addExpense = async (req, res) => {
  ...
  expensesController.getAll = async (req, res) => {
    ...
    console.log('length',length);    let history = req.body.history;
    ...
    if(obj.price || obj.item || obj.quantity || obj.details)
    {
      ...
      try {
        ...
        searhItem = await Expenses.find(queryObject).populate('userId').skip(start*length).limit(length)
        .populate('item');
        counts = await Expenses.countDocuments(queryObject);
        if (history == 'true') {
      ...
      let items;
      try {
        items = await Expenses.find({}).populate('userId').skip(start*length).limit(length)
        .populate('item');
        counts = await Expenses.countDocuments();
        if (history == 'true') {
          delete req.body.history;
          result = await getRevision(req.body).then((value) => {
            return value;
