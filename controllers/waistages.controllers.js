  ...
  waistagesController.getAll = async (req, res) => {
    ...
    if(obj.buyer || obj.item || obj.quantity || obj.reason)
    {
      ...
      try {
        ...
        searhItem = await Waistages.find(queryObject).populate('userId').skip(start*length).limit(length)
        .populate('buyer')
        .populate('item');
        counts = await Sales.countDocuments(queryObject);
      ...
      let items;
      try {
        items = await Waistages.find({}).populate('userId').skip(start*length).limit(length)
        .populate('buyer')
        .populate('item');
        counts = await Sales.countDocuments({});
